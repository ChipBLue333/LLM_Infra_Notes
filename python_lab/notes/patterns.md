...
## 2026年6月14日: Trie + 二维网格 DFS

### 适用场景

在二维字符网格中同时搜索大量候选单词，并且：
...
1. 把所有候选单词插入 Trie。
2. 从网格中的每个格子开始 DFS。
3. 当前字符不在 Trie 当前节点的 `children` 中时立即剪枝。
4. 进入对应 Trie 子节点后，检查是否到达完整单词。
5. 临时标记当前格子，递归搜索四个方向，最后恢复。
...
- 进入 DFS 时，当前路径之前使用的格子已经被标记。
- `node` 对应当前网格路径匹配到的 Trie 前缀位置。
- DFS 离开当前格子前，必须恢复其原始字符。
- 找到完整单词后仍然继续搜索，因为该单词可能是更长单词的前缀。

### 去重方法

方法一：使用 `set` 保存找到的单词。

方法二：在 Trie 节点中保存完整单词，找到后清空：
...
则可以从父节点的 `children` 中删除该分支，减少后续 DFS。

### 易错点

- 对每个单词分别运行一次普通 Word Search，失去 Trie 共享前缀的优势。
- 找到较短单词后立即返回，漏掉以它为前缀的更长单词。
- 标记格子后忘记恢复，影响其他搜索路径。
- 允许斜线移动，或者在同一条路径中重复使用格子。
- 只判断 Trie 路径存在，没有判断节点是否对应完整单词。

### 复杂度

设所有候选单词字符总数为 `S`，网格大小为 `M x N`，最长单词长度为 `L`。

- 构建 Trie：Time `O(S)`，Space `O(S)`。
- 搜索最坏：Time `O(MN * 4 * 3^(L - 1))`。
- 递归栈：Space `O(L)`。
- Trie 前缀剪枝通常会显著减少实际访问的搜索状态。
